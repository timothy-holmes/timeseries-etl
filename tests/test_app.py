...
def test_app(mock_app_engine, mock_bom, mock_p110, mock_log):
    ...
    schedule.every(1).seconds.do(bom_job, bom_service=mock_bom(log), engine_service=engine)
    schedule.every(1).seconds.do(p110_job, p110_service=mock_p110(log), engine_service=engine)
    ...
    try:
        for i in range(1,10):
            ...
            if i == 3: raise KeyboardInterrupt # 1+3 times through loop
